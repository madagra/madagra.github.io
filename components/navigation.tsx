...
export function MediumLink() { return <a className="medium-link" href={mediumLink.href}><svg viewBox="0 0 24 24" aria-hidden="true"><path d={mediumLink.path} /></svg><span>Medium</span></a>; }

export function SocialLinks() { return <div className="social-links">{socialLinks.map((link) => <a key={link.label} className={link.label === "LinkedIn" ? "linkedin-icon" : undefined} href={link.href} aria-label={link.label} title={link.label}><svg viewBox="0 0 24 24" aria-hidden="true"><path d={link.path} /></svg></a>)}</div>; }
